window.addEventListener('load', function() {
  ...
  setInterval(function(){
    var position = document.getElementsByClassName("header")[0].getBoundingClientRect();
    mouse.x = position.left;
    mouse.y = position.top;
    console.log(mouse);
  }, 1);
	...
	class Particle {
		...
		update() {
			...
			if (this.size < 0) {
				...
				this.weight = (Math.random(2, 5) + 0.1);
			}
			this.y += this.weight;
			this.weight += 0.325;
		...
		for (let a = 0; a < particleArray.length; a++) {
			for (let b = a; b < particleArray.length; b++) {
				...
				if (distance < 110) {
					...
					ctx2.strokeStyle = 'rgb(102,0,0,' + opacityValue + ')';
					ctx2.beginPath();
					ctx2.lineWidth = 2;
